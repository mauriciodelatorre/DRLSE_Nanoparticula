...
#el tamaño máximo ocurre a los 30 segundos, despues aumenta el número de semillas. 

#cargar imagen
imagen = imread('sim30 (2.1) bigjpg.jpg', True)
...
def grfc(v: np.ndarray):
    ...
    y = np.arange(0, y, 1)
    X, Y = np.meshgrid(x, y)
    ax1.plot_surface(X, Y, -v, rstride=2, cstride=2, color= 'g',cmap=plt.get_cmap('magma'), linewidth=0, alpha=0.6, antialiased=True)
    ax1.contour(X, Y, v, 0, colors= 'b', linewidths=2)



#función que dibuja la ecuación level set
#pause: controla la velocidad de la animación 0.003
...
def Parametros():    
    ...
    #iniLSF[28:33, 48:53] = -c0  #para semilla30 (2)
    #iniLSF[150:155, 170:175] = -c0 # para semilla30 (2.1)
    iniLSF[90:95, 105:110] = -c0  # semilla30 (2.1) bigjpg.com
    ...
    return {
        ...
        'iter': 8,        #10  o 15
        'iterMax': 30,       #30
        'lamda': 2,  # 1.6  0 5.5 coeficiente del largo L(v)
        'alfa': -2.5,  # -3 coeficiente del termino de area A(v)
        'epsilon': 2,  # 1.5 recomendado (pero genera perturbaciones numéricas) parámetro para el delta de Dirac
            }
